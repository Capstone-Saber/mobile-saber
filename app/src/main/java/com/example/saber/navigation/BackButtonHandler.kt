...
private val LocalBackPressedDispatchar = staticCompositionLocalOf <OnBackPressedDispatcherOwner?>{null}
...
internal fun ComposableHandler(
    ...
    val dispatcher = (LocalBackPressedDispatchar.current ?: return).onBackPressedDispatcher
...
internal fun SystemBackButtonHandler(onBackPressed: () -> Unit) {
    CompositionLocalProvider(
        LocalBackPressedDispatchar provides LocalLifecycleOwner.current as ComponentActivity
    ) {
        ComposableHandler {
            onBackPressed()
        }
